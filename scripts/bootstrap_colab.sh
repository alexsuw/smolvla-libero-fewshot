...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  echo "Usage: scripts/bootstrap_colab.sh [--output-dir PATH]"
  echo "Mount Google Drive first, then sync and validate the pinned Colab runtime."
  exit 0
fi

if [[ "$(uname -s)" != "Linux" ]]; then
  echo "bootstrap_colab requires Linux; current host is $(uname -s)." >&2
  exit 2
fi
